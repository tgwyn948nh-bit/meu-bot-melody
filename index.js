...
// =========================================================
// TOKEN DO DISCORD (Configurado diretamente para o Render)
// =========================================================
const TOKEN = 'MTUxOTgzNTQ1NTM5NzU2NDQ4OA.GFIqrD.VKbhj76kgWO17SgKsyC1UwWHXXxa9UoesW4BB4';

// ID do canal privado de logs do administrador
const CANAL_LOG_ID = '1519852269548474469';

// ---------------------------------------------------------
// Express — mantém o bot online (Obrigatório para o Render)
// ---------------------------------------------------------
const app = express();
  ...
  if (message.content.startsWith('!loja')) {
    ...
    if (status === 'aberta') {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription('O status foi updated para **Loja Aberta! 🟢**')
        .setColor('#00FF7F')
        .setFooter({ text: 'My Melody🍓' });
      return message.reply({ embeds: [embed] });
    }
    ...
    if (status === 'fechada') {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription('O status foi updated para **Loja Fechada 🔴**')
        .setColor('#FF4444')
        .setFooter({ text: 'My Melody🍓' });
      return message.reply({ embeds: [embed] });
    }
...
// ---------------------------------------------------------
// Login automático com o Token informado
// ---------------------------------------------------------
client.login(TOKEN);
